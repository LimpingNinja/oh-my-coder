...
/**
 * Preserved from pi-vscode: reports a terminal session mapping.
 *
 * Terminal id is NOT durable session identity in OMP; this method exists for
 * bridge extension compatibility. OMP session identity uses JSONL path/session id.
 * De-emphasized but not removed — the runtime-side bridge extension may still call it.
 */
